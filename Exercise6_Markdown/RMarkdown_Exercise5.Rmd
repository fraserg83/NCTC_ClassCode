...
To make an ordered list, place each item on a new line after a number followed by a period followed by a space, like this:   
1. item 1
2. item 2
3. item 3
...
[Working with Images in RMarkdown](http://blog.revolutionanalytics.com/2017/06/rmarkdown-tricks.html){:target="_blank"}
...
presbyterian <- mydata %>% 
  ...
  arrange(desc(Congregations,`Religious Group`))
